...
for ($i = 0; $i < count($city); $i++) {
    $city[$i][2]=96.1055493 - $city[$i][2];
    print_r("[\"{$city[$i][0]}\", {$city[$i][1]}, {$city[$i][2]}],\n");
}
function createMapInFile($stantions,$fileName)
{
    ...
    for ($i = 0; $i < count($stantions); $i++) {
        $x = 150 + 1200 * (($maxX - $stantions[$i][2]) / ($maxX - $minX));
    ...
    for ($i = 0; $i < count($stantions); $i++) {
        $x = 160 + 1200 * (($maxX - $stantions[$i][2]) / ($maxX - $minX));
        $y = 50 + 450 * (($maxY - $stantions[$i][1]) / ($maxY - $minY));
